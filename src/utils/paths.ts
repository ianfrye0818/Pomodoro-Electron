...
export async function ensureIdeationDir(projectPath: string): Promise<string> {
  ...
  await fs.mkdir(getIdeasDir(projectPath), { recursive: true });
  await fs.mkdir(getIdeationSessionsDir(projectPath), { recursive: true });
  await fs.mkdir(getIdeationDraftsDir(projectPath), { recursive: true });
  return ideationDir;
}
